...
class CNN_VANetwork(nn.Module):
    def __init__(self, input_shape, output_size):
        super(CNN_VANetwork, self).__init__()
        c, h, w = input_shape
        ...
        self.val_fc1 = nn.Linear(conv_out_size, 128)
    ...
    def forward(self, obs):
        ...
        advantage_fc1 = F.relu(self.adv_fc1(x))
        advantage = self.adv_fc2(advantage_fc1)

        value_fc1 = F.relu(self.val_fc1(x))
...
class DuelingDQNAgent:
    def __init__(self, obs_shape, action_size, config):
        ...
        self.q_network = CNN_VANetwork(obs_shape, action_size).to(self.device)
        self.t_network = CNN_VANetwork(obs_shape, action_size).to(self.device)
    ...
    def update_policy(self):
        ...
        max_next_q_values = self.t_network(next_obss).max(dim=1)[0].detach().unsqueeze(1)
    ...
    def save_model(self, fpath):
        ...
        torch.save(self.t_network.state_dict(), f"{fpath}/DQN_checkpoint.pt")

    def load_model(self, fpath):
        self.t_network.load_state_dict(torch.load(f"{fpath}/DQN_checkpoint.pt",
...
class MAIN:
    ...
    def run_sweep_agent(self):
        ...
        # --- Load agent, trainer ---
        agent = DuelingDQNAgent(cfg['obs_shape'], cfg['action_dim'], cfg)
        trainer = Trainer(cfg)
