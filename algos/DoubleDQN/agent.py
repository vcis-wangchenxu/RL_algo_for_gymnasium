...
class Agent:
    def __init__(self, cfg) -> None:
        ...
        self.policy_net = Qnet(h, w, self.action_dim).to(self.device)
        self.target_net = Qnet(h, w, self.action_dim).to(self.device)
    ...
    def update_policy(self):
        ...
        # [actions]: next_states max Q-values from the policy_net;
        # and find the corresponding Q-values of [actions] in the target_net
        max_policy_q_actions = self.policy_net(next_states).max(1)[1].unsqueeze(1)
        next_target_q_values = self.target_net(next_states).gather(1, index=max_policy_q_actions).detach()
    ...
    def save_model(self, fpath):
        ...
        torch.save(self.target_net.state_dict(), f"{fpath}/DoubleDQN_checkpoint.pt")

    def load_model(self, fpath):
        self.target_net.load_state_dict(torch.load(f"{fpath}/DoubleDQN_checkpoint.pt",
